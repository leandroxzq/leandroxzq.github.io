  ...
  return (
    <Page>
      ...
      <ContainerMotion padding="4" delay={0.3}>
        <h3 className={`text-lg font-bold text-white md:text-xl ${pressStart2P.className}`}>
          {content.about.education.title}
        </h3>

        {content.about.education.degrees.map((degree, index) => (
          <div key={index}>
            <h4 className="text-md mt-4 font-semibold underline">{degree.school}</h4>
      ...
      <ContainerMotion padding="4" delay={0.5}>
        <div className="flex flex-col space-y-4">
          <h3 className={`text-md font-bold text-white md:text-lg ${pressStart2P.className}`}>
            {content.about.contact}
          </h3>
          {Links.map((link, index) => (
            <div
              key={index}
              className="bg-gradient-light flex w-fit flex-row gap-2 rounded-md bg-gray-300 p-2 font-semibold text-black hover:text-blue-600"
            >
              <Tooltip
